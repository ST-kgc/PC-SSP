...
def get_arguments():
    ...
    parser.add_argument("--data-dir", type=str, default="./generate_path/data/FB15k-237/", help="FB15k-237 or WN18RR")
    parser.add_argument('-logdir', dest="log_dir", default='./log/', help='Log directory')
    parser.add_argument("--snapshot-dir", type=str, default="./snapshot", help="")
    parser.add_argument("--train-dataset", type=list, default=['path_store_train_2hops.pt'], help="")
    ...
    parser.add_argument("-mode", type=str, default='Joint',  help="set Model: P2E, P2P or Joint")
...
class Main(object):
    ...
    def __init__(self, args):
        ...
        self.load_data(args)
        if  self.p.mode == 'P2E':
            self.model = P2E(self.ent_vocab_size, self.rel_vocab_size, 200, 200, 5, self.k_size, 5).to(self.device)
            self.optimizer = optim.Adam(
                filter(lambda p: p.requires_grad, self.model.parameters()),
                lr=0.0001, betas=(0.9, 0.98), eps=1e-09, weight_decay=0, amsgrad=True)
            total = sum([param.nelement() for param in self.model.parameters()])
            print("Number of parameter: %.2fM" % (total / 1e6))
        elif self.p.mode == 'P2P':
            self.model = P2P(self.ent_vocab_size, self.rel_vocab_size, 2000,2000,2000, 5, self.k_size, 11,self.p.bias).to(self.device)
            self.optimizer = optim.Adam(
                filter(lambda p: p.requires_grad, self.model.parameters()),
            ...
            self.model_w = P2E(self.ent_vocab_size, self.rel_vocab_size,
                                  200, 200, 5,
            ...
            self.optimizer = optim.Adam([{'params': self.model_w.parameters(), 'lr':0.001},
                                                 {'params': self.model_s.parameters(), 'lr': 0.00001}],
    ...
    def train(self, step, epoch, log_interval):
        ...
        if args.mode == 'P2E' or args.mode == 'P2P':
            self.model.train()
    ...
    def fit(self):
        ...
        if self.p.restore:
            ...
            pprint(test_results)
        run_name = "pc-ssp" + time.strftime("-%Y-%m-%d_%H_%M_%S")
