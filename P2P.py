...
class Conve_Encoder(nn.Module):###genc
    ...
    def concat(self, x):
        B = x.shape[0]
        x = x.view(-1, self.sentence_len, self.hidden_dim)
...
class P2P(nn.Module):
    ...
    def forward(self, x, device):
        ...
        batch = x.shape[0]
        hidden = self.init_hidden(batch, device)
        x = self.get_sentence_embedding(batch, x, device)

        # get path embeddings
        z = self.get_path_embedding(x)  # z.shape: (B,W,D)
        ...
        pred = torch.empty((self.k_size, batch, self.enc_dim), dtype=torch.float) # pred (empty container).shape: (W2,B,D)
